...
app.get('/nueva_orden', mdi, (req, res) => {
  ...
  let json = {};
  let qry = `SELECT TOP 1 r.id_toma
             ...
             WHERE (t.id_toma_random = '${id_toma_random}' AND t.id_toma_consecutivo = ${id_toma_consecutivo})
               AND (r.id_examen = (SELECT TOP 1 codigo_covid FROM [192.168.10.4\\SQL2008].CitasCovid.dbo.configuracion))
               AND (r.id_valor = 50557)`;
  sql_lis.query(qry)
    .then( ([results, metadata]) => {
    ...
    .finally( () => {
      res.json( json );
      if (json.id_toma > 0) {  // En caso de que la orden se haya encontrado y tiene COVID
...
app.get('/ordenes', mdi, (req, res) => {
  ...
  let qry = `SELECT
             ...
             FROM LPCV_RESULTADOS r (NOLOCK)
                ...
                LEFT JOIN [192.168.10.4\\SQL2008].CitasCovid.dbo.indre ON t.id_toma = indre.id_toma
             WHERE (r.id_examen = 10020)
                AND (r.id_valor = 50557) `;
...
// Ruta para leer resultados por estudio
app.use("/resultados/:id_examen", mdi, (req, res) => {
  let json = {};
  let qry = `
    SELECT
